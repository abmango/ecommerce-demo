<template>
  ...
  <div class="container mx-auto p-8 mt-20">
    ...
    <form @submit.prevent="submitCreation" :disabled="form.processing" class="bg-white p-6 rounded-lg shadow-md max-w-lg mx-auto">
      ...
      <div class="mb-2">
        ...
        <TextInput  @keyup="checkIsRequired('name')"
          id="name" v-model="form.name" type="text" class="mt-1 block w-full" autocomplete="name"
          placeholder="Nombre del producto" />
        <InputError v-if="form.errors.name" class="mt-2" :message="form.errors.name" />
      </div>
      <div class="mb-2">
        <InputLabel for="price" value="Precio" />
        <TextInput  @keyup="checkIsRequired('price')"
          id="price" type="number" v-model="form.price" class="mt-1 block w-full" autocomplete="price"
          placeholder="0.00" />
      ...
      <div class="mb-2">
        ...
        <TextInput  @keyup="checkIsRequired('stock')"
          id="stock" type="number" v-model="form.stock" class="mt-1 block w-full" autocomplete="stock"
          placeholder="0" />
        <InputError v-if="form.errors.stock" class="mt-2" :message="form.errors.stock" />
      </div>
      <div class="mb-2">
        <InputLabel for="tipo" value="Tipo de producto" />
        <TextInput  @keyup="checkIsRequired('type')" :disabled="form.processing"
          id="tipo" v-model="form.type" type="text" class="mt-1 block w-full" autocomplete="tipo"
